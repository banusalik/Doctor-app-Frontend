      ...
      if (responseData.status) {
        ...
        return doctorsData.map((doctor) => ({
          ...
          Doctor_Name: doctor.Doctor_Name
            ? doctor.Doctor_Name
            : "Unknown Doctor",
          Speciality: doctor.Bio ? doctor.Bio : "Unknown Specialty",
          Rating: doctor.Rating ? doctor.Rating : 0,
          Address: doctor.Address ? doctor.Address : "Unknown Hospital",
          // Add other properties as needed
        }));
      } else {
        throw new Error(responseData.message || "Invalid doctor data format");
      }
    } catch (error) {
    ...
    <Layout>
      ...
      <div className="min-h-[80vh]  mb-4 pt-6">
        ...
        <div className="max-w-screen-xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {/* Map through doctors and render HighlyRatedDoctor component */}
          {doctors.map((doctor, index) => (
            <HighlyRatedDoctor key={index} doctor={doctor} />
          ))}
        </div>
      </div>
      {/* FAQ */}
